# 👋 Hi, I'm Karan Ohol  

🎓 I'm a **2nd Year B.Tech Student in Artificial Intelligence & Data Science (AI & DS)**  
💻 Passionate about **Machine Learning, Data Analysis, and Embedded Systems**  
⚙️ Skilled in **Python, C, and Arduino Programming**

---

### 🚀 About Me
- 🌱 Currently learning **Machine Learning & Data Structures**
- 🧠 Exploring **AI model development** and **automation projects**
- 🔬 Interested in combining **software and hardware** (IoT + AI)
- 🤝 Open to **collaboration on Python, AI, and IoT-based projects**

---

### 🛠️ Languages & Tools
| Python | C | Arduino | Git | VS Code | Jupyter |
|:------:|:--:|:--------:|:--:|:-------:|:--------:|
| 🐍 | ⚙️ | 🤖 | 🌿 | 💻 | 📘 |

---

### 📊 GitHub Stats
![Karan's GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=radical)

---

### 🧠 Featured Projects
- 🔍 [AI Sensor Project](#) — Arduino + Motion Detection  
- 📈 [Data Analysis with Python](#) — Pandas, NumPy, Matplotlib  
- 🧩 [C Programming Examples](#) — DSA & logic building

---

### 📫 Connect with Me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin)](#)  
[![Gmail](https://img.shields.io/badge/Gmail-red?style=for-the-badge&logo=gmail)](#)

---
⭐ **“Code. Learn. Build. Repeat.”**
